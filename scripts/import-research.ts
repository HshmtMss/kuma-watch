...
/**
 * Drive 由来の本文中に出現する「...である1。」「...更新している 1。」など
 * bare 参照番号を「...である（※1）。」に整形する。学術日本語慣行の ※N 形式。
 *
 * - 日本語/閉じ括弧 (オプションで空白) + 1-3 桁数字 + 句読点 を対象に。
 * - 句読点自体は残し、数字部分だけを （※N） に置換する。
 * - 「2025年」「5月9日」のように年・月・日で閉じる数字は対象外 (regex が句読点を要求するため)。
 */
