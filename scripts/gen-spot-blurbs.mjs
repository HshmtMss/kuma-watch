...
function promptFor(batch) {
  // 出没件数は渡さない。渡すと紹介文に必ず書き込まれ、施設紹介と件数が一続きの
  // 地の文になる（2026-09-04 に施設運営者から「施設自体が危険に読める」と指摘）。
  // 件数の提示は RiskBanner・地図・一覧の担当で、紹介文は地点そのものの説明に絞る。
  const lines = batch.map((r, i) => {
    return `[${i}] 名称:「${r.name}」 種別:${CATJP[r.cat]} 所在:${r.pref}${r.muni ? "/" + r.muni : ""}`;
  }).join("\n");
  return `あなたは日本の旅行・アウトドア情報サイトの編集者です。各スポットについて、来訪者向けの「表示名」と「紹介文」を作成してください。

厳守事項:
- 事実を創作しない。名称・種別・所在から確実に言えることだけ書く。具体的な歴史・施設名・料金・アクセスは推測で書かない。
- クマ出没の件数・有無には一切触れない。「周辺で◯件」「出没情報はありません」等を書かない。
  出没状況はページ上部の状況カードと地図が担当する。紹介文にまで書くと、施設の紹介と
  出没情報が一続きの文になり「その施設自体が危険」と読めてしまう（2026-09-04 に施設
  運営者から指摘を受けた実例あり）。
- 「危険」「警戒」等の煽る語は使わない。「早朝・夕方は音を出す」程度の一般的な注意を
  1文添えるのは可。
