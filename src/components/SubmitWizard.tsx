 ...
 * クマ目撃投稿ウィザード（4 ステップ）。
 *
 * Web の /submit と LINE 内 (LIFF) の /line/submit の両方から使う共有部品。
 * 差分は props で吸収する:
 *   - enableMapPick: 「地図で選ぶ」導線の可否。地図ピッカーは /submit に戻る
 *     作りなので (KumaClient)、LIFF (/line/submit) では戻れない。LIFF では
 *     false にして現在地 (GPS) 一本に寄せる。
 *   - idToken: LINE の idToken。渡すと投稿に添えてサーバで userId を紐付け、
 *     将来「承認されました」通知や連投制限に使う (本人には匿名のまま)。
 */
...
export type SubmitWizardProps = {
  /** URL クエリ等で最初から座標が分かっている場合。無ければ null。 */
  initialLat?: number | null;
  initialLon?: number | null;
  /** 地図ピッカー (/?pick=submit) から戻ってきた回。下書きを復元する。 */
  fromPicker?: boolean;
  /** 「地図で選ぶ」を出すか。LIFF では地図ピッカーに戻れないため false。 */
  enableMapPick?: boolean;
  /** LINE の idToken。渡すと投稿に添えてサーバで userId を紐付ける。 */
  idToken?: string | null;
};
