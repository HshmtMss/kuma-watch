...
export default async function OGImage({ params }: Props) {
  ...
  const summary = PREF_NAMES.has(pref) ? await getPrefSummary(pref) : null;

  const cityCount = summary?.cityCount ?? 0;
  const totalCount = summary?.totalCount ?? 0;
  const count90d = summary?.count90d ?? 0;
  const latest = formatDate(summary?.latestDate ?? null);
      ...
      >
        ...
        >
          <div
            style={{
              display: "flex",
              fontSize: pref.length > 4 ? 116 : 132,
              fontWeight: 700,
              color: "#111827",
              lineHeight: 1.1,
            }}
        ...
        <div style={{ display: "flex", gap: "20px", marginTop: "auto" }}>
          ...
          >
            <div style={{ display: "flex", fontSize: "18px", color: "#6b7280" }}>
              市町村数
            </div>
            <div
              style={{
