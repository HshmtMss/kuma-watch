...
type HistRow = { date: string; totalSubscribers: number };
...
function HistoryChart({ history }: { history: HistRow[] }) {
  ...
  const max = Math.max(1, ...history.map((h) => h.totalSubscribers));
  const niceMax = max <= 5 ? max : Math.ceil(max / 5) * 5;
  const x = (i: number) => PADL + (i / (n - 1)) * plotW;
  const y = (v: number) => PADT + (1 - v / niceMax) * plotH;
  const pts = history.map((h, i) => `${x(i)},${y(h.totalSubscribers)}`).join(" ");
  const first = history[0];
  const latest = history[n - 1];
  const delta = latest.totalSubscribers - first.totalSubscribers;
